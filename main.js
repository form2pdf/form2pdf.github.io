...
function calculateFontPxX(fontSize, text) {
  // TODO, based on font type, characters, etc., calculat the X size
  return fontSize * text.length * 0.65;
}

function calculateFontPxY(fontSize) {
  return fontSize * 1.1;
}
...
async function createPdf() {
  ...
  // const logoImageBytes = await fetch(logoImageUrl).then((res) =>
  //   res.arrayBuffer()
  // );
  // const logoImage = await pdfDoc.embedJpg(logoImageBytes);

  // calculate the position os _headers_pn to align right and (possibly) fill CMI
  yPointer = height - marginTop - calculateFontPxY(fontSizeH);
  xPointer = width - marginSides - calculateFontPxX(fontSizeH, _header_p0);
  currentPage.drawText(_header_p0, {
    x: xPointer,
    y: yPointer,
    size: fontSizeH,
    font: fontBold,
  ...
  yPointer = yPointer - calculateFontPxY(fontSizeT) - lineSpacing;
  xPointer = width - marginSides - calculateFontPxX(fontSizeT, _header_p1);
  currentPage.drawText(_header_p1, {
    x: xPointer,
    y: yPointer,
    size: fontSizeT,
    font: font,
  ...
  yPointer = yPointer - calculateFontPxY(fontSizeT) - lineSpacing;
  xPointer = width - marginSides - calculateFontPxX(fontSizeT, _header_p2);
  currentPage.drawText(_header_p2, {
    x: xPointer,
    y: yPointer,
    size: fontSizeT,
    font: font,
  ...
  yPointer = yPointer - calculateFontPxY(fontSizeT) - paragraphSpacing;
  xPointer = marginSides;

  for (let index = 0; index < _body_cl.length; index++) {
    const element = _body_cl[index];

    // check spacing to separate words, calculate height, and construct our paragraph page,
    // in memory (i.e., a variable), then write the whole chunk based on yPointer, xPointer
    // and increment those to the next paragraph.
    currentPage.drawText(element.clause, {
      x: width / 2,
      y: yPointer,
      size: fontSizeT,
      font: font,
      color: PDFLib.rgb(0, 0, 0),
      maxWidth: width - marginSides,
      lineHeight: calculateFontPxY(fontSizeT),
    });

    yPointer = yPointer - calculateFontPxY(fontSizeT) - lineSpacing;

    currentPage.drawText(element.title, {
      x: width / 2,
      y: yPointer,
      size: fontSizeT,
      font: font,
    ...
    yPointer = yPointer - 10 * calculateFontPxY(fontSizeT) - 10 * lineSpacing; // calculate number of lines
  }
  ...
  currentPage.drawText(_footer_p2, {
    x: 0,
    y: 100,
    size: fontSizeH,
    font: fontBold,
...
const _body_pre = `Entre:
Desfecho Fidedigno Unipessoal Lda, com sede social na R. do Olival nº9 1ºEsq, B. Mira Sul, 3660-495 em S. Pedro Sul, e com o NIF 517 351 455, detentora da licença AMI n.º 21936, emitida pelo Instituto dos Mercados Públicos, do Imobiliário e da Construção I. P. (IMPIC, I. P.), adiante designada como Mediadora,
E
(nome do cliente)_______________________________________________________, aqui representado por (nome do procurador, se o houver) ____________________________(estado civil) ________________, sob o regime de bens __________________, com (cônjuge) ________________________________________, residente(s) na _____________________________________________, em ____________, portador(es) do(s) CC/BI nºs _______________ e _______________ e contribuinte(s) fiscal(is) nºs _______________ e _______________ ,com telemóvel nº __________ e __________ e e-mail ____________________________, adiante designado(s) como Segundo(s) Contratante(s) na qualidade de _______________ (Proprietário; Senhorio; Trespassante; Outro), é celebrado o presente Contrato de Mediação Imobiliária que se rege pelas seguintes cláusulas:
`;
...
const _body_cl = [
  {
    ...
    text: `O Segundo Contratante é proprietário e legítimo possuidor da fração autónoma/prédio (rústico/urbano)/estabelecimento comercial, destinado(a) a _______________ , sendo constituído por ___ divisões assoalhadas, com área total de ______ m2, sito na (Rua, Avenida) ________________________________________________, em ____________, (freguesia) ____________, (concelho) ____________, descrito na Conservatória do Registo Predial de ____________, sob a ficha n.º ______, com licença de construção/utilização n.º ____________, emitida pela Câmara Municipal de ____________, em ___/___/_____ e inscrito na matriz predial (urbana/rústica) com o artigo n.º ___ da freguesia de ____________/ omisso na matriz ___________. O imóvel com o Certificado de Desempenho Energético e da Qualidade do Ar Interior com o Nº do CER CE __________________, emitido em ___/___/______, por _____________________ - Perito nº ______________ (quando aplicável)`,
  },
  {
    clause: "Cláusula 2.ª",
    title: "Identificação do Negócio",
    text: `1 - A Mediadora obriga-se a diligenciar no sentido de conseguir interessado na __ Compra __ Trespasse __ Arrendamento, pelo preço de _______________ Euros (________________________________________________) desenvolvendo para o efeito ações de promoção e recolha de informações sobre os negócios pretendidos e características dos respetivos imóveis.
    2 - Qualquer alteração ao preço fixado no número anterior deverá ser comunicada de imediato e por escrito à Mediadora.`,
  },
];

const _body_rest = `Cláusula 3.ª
Ónus e Encargos
(_) O imóvel encontra-se livre de quaisquer ónus ou encargos.
(_) O Segundo Contratante declara que sobre o imóvel descrito na cláusula 1.ª recaem os seguintes ónus e encargos (hipotecas e penhoras) _____________ pelo valor de _______________ Euros a favor de ______________________.
...
2 - A remuneração pelos serviços referidos no número anterior considera-se incluída no montante acordado na cláusula 5.ª e só será devida nos termos aí descritos.
...
Para garantia da responsabilidade emergente da sua atividade profissional, a Mediadora celebrou um contrato de seguro obrigatório de responsabilidade civil no valor de 150.000,00 Euros (cento e cinquenta mil euros), apólice n.º 0007902198, através da seguradora TRANQUILIDADE com sede na Av. da Liberdade, 242 1250-149 Lisboa.
...
Na preparação do presente contrato de mediação imobiliária colaborou o angariador imobiliário (nome) ___________________________, portador do Cartão de Cidadão/Bilhete de Identidade n.º ___________ e/ou Contribuinte Fiscal n.º___________.

Cláusula 11.ª
Foro Competente
Para dirimir quaisquer litígios emergentes da execução do presente contrato, as partes acordam entre si estabelecer como competente o Foro da Comarca de ___________, com a expressa renúncia a qualquer outro.
(Esta cláusula é facultativa e só deverá ser preenchida se as partes assim o pretenderem e acordarem.)

Cláusula 12.ª
Resolução Alternativa de Litígios
1 - Nos termos do disposto no artigo 18.º da Lei n.º 144/2015, de 8 de setembro, na redação atual, em caso de litígio ou insatisfação com o serviço prestado poderá o Segundo Contratante recorrer ao Centro de resolução alternativa de litígios de consumo___________________ com o sítio eletrónico na Internet ___________________ ,de que a Mediadora é aderente.
